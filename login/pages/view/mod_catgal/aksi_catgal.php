...
if (empty($_SESSION['namauser']) AND empty($_SESSION['passuser'])){
	...
	// Input catgal
elseif ($module=='catgal' AND $act=='update2')
{
...
elseif ($module=='catgal' AND $act=='input'){
	...
	if (!empty($lokasi_file)){
		if ($tipe_file != "image/jpg" AND $tipe_file != "image/jpeg" AND $tipe_file != "image/png"){
			...
			$sSQL = "insert into tb_catgal(Title,Detail,Date,Imgpath,tip,Title_id,Detail_id) 
			values('$_POST[Title]','".$_POST[Detail]."','$tgl','$nama_file_unik2','$_POST[tip]','$_POST[Title_id]','".$_POST[Detail_id]."')";
			mysql_query($sSQL) or die($sSQL);
			$loc = '../../../?module='.$module;
			echo "<script>document.location = '$loc'</script>";
		}
	} else {
		$sSQL = "insert into tb_catgal(Title,Detail,Date,Subtitle,tip,Title_id,Detail_id) 
		values('$_POST[Title]','".$_POST[Detail]."','$tgl','".mysql_real_escape_string($_POST[Subtitle])."','$_POST[tip]','$_POST[Title_id]','".$_POST[Detail_id]."')";
		mysql_query($sSQL);
		$loc = '../../../?module='.$module;
		echo "<script>document.location = '$loc'</script>";
	}
	...
	if (empty($lokasi_file)){
		...
		if ($tipe_file != "image/jpg" AND $tipe_file != "image/jpeg" AND $tipe_file != "image/png"){
			echo "<script>window.alert('Upload Gagal, Pastikan File yang di Upload bertipe *.JPG');
				window.location=('../../../?module=catgal)</script>";
		} else {			
...
elseif ($module=='catgal' AND $act=='hapusgambar'){ 
	...
	$real_Imgpath = trim($real_Imgpath);
	unlink($remove);  
	header('location:../../../?module='.$module.'&act=editcatgal&id='.$_GET[id]);
}
}