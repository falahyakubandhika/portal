              ...
              <div class="card card-statistics">
                <div class="card-body">
                  <div class="clearfix">
                      ...
                      <?php
$aksi="pages/view/mod_catgal/aksi_catgal.php";
if (isset($_GET['act'])){
    switch($_GET['act']){
  // Tampil users_group
    // Tampil User
  ...
  echo "<h2>New Gallery</h2>
   ...
   case "editcatgal":
    ...
    echo "<form method=POST action='$aksi?module=catgal&act=update' enctype='multipart/form-data' id='Form'  onSubmit='return CekUpdate();' name=formData>
      ...
      <tr><td>Content Id</td>  <td> <textarea id='loko2' name='Detail_id'  style='width: 710px; height: 350px;'>$r[Detail_id]</textarea></td></tr>";
  ...
  echo "<h2>Gallery List</h2>
          ...
          <button  style='margin-bottom:15px' type='button' onclick=\"window.location.href='?module=catgal&act=tambahcatgal ';\" class='btn btn-primary btn-fw'> <i class='mdi mdi-file-plus'></i>Add Gallery</button>
          ...
          "; 
    $tampil=mysql_query("SELECT * FROM tb_catgal order by gal ");
    $no=1;
    ...
    if ($r['fl_active'] == '1'){
      ...
      <td>$blokir</td>";   
     echo "
             <td><a href=?module=catgal&act=editcatgal&id=$r[gal] title='Edit'><button type='button' class='btn btn-dark btn-fw'>
                          <i class='mdi mdi-cloud-download'></i>Edit</button></a>  
          <a href=$aksi?module=catgal&act=hapus&id=$r[gal] title='Delete'><button type='button' class='btn btn-danger btn-fw'> X Delete</button></a>
           </td>";
