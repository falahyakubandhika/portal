              ...
              <div class="card card-statistics">
                <div class="card-body">
                  <div class="clearfix">
                      ...
                      <?php
$aksi="pages/view/mod_csr/aksi_artikel.php";
if (isset($_GET['act'])){
    switch($_GET['act']){
  // Tampil users_group
    // Tampil User
  ...
  case "tambahartikel": 
  echo "<h2>New CSR</h2>
    <form method=POST action='$aksi?module=csr&act=input' enctype='multipart/form-data' id='FormData' onSubmit='return CekField();' name=formData>
   ...
   case "editartikel":
    ...
    $edit = mysql_query("SELECT * FROM tb_csr WHERE id_article='$_GET[id]' AND username='$_SESSION[namauser]'");
    $r    = mysql_fetch_array($edit);
    echo "<form method=POST action='$aksi?module=csr&act=update' enctype='multipart/form-data' id='Form'  onSubmit='return CekUpdate();' name=formData>
      ...
      if ($r['gambar']!=''){
        ...
        echo "<a href='$aksi?module=csr&act=hapusgambar&id=$r[id_article]&namafile=$r[gambar]'><img src='images/cross.png' class='tombol'></a>";
      }
      echo "</td></tr>
  ...
  echo "<h2>CSR List</h2>
          ...
          <button  style='margin-bottom:15px' type='button' onclick=\"window.location.href='?module=csr&act=tambahartikel ';\" class='btn btn-primary btn-fw'> <i class='mdi mdi-file-plus'></i>Add CSR</button>
          ...
          "; 
    $tampil=mysql_query("SELECT * FROM tb_csr ORDER BY tanggal DESC ");
    $no=1;
    ...
    if ($r['fl_active'] == '1'){
      ...
      <td>$blokir</td>";   
     echo "
             <td><a href=?module=csr&act=editartikel&id=$r[id_article] title='Edit'><button type='button' class='btn btn-dark btn-fw'>
                          <i class='mdi mdi-cloud-download'></i>Edit</button></a>  
          <a href=$aksi?module=csr&act=hapus&id=$r[id_article] title='Delete'><button type='button' class='btn btn-danger btn-fw'> X Delete</button></a>
           </td>";
