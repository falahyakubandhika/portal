...
if (empty($_SESSION['namauser']) AND empty($_SESSION['passuser'])){
	...
	$act=$_GET['act'];
	// Hapus artikel
	if ($module=='csr' AND $act=='hapus'){
		$data=mysql_fetch_array(mysql_query("SELECT gambar,fl_active FROM tb_csr WHERE id_article='$_GET[id]'"));
			if ($data['gambar']!=''){
				$gambar = $data['gambar'];
				$remove = "../../../".$gambar;
				...
				mysql_query("DELETE FROM tb_csr WHERE id_article='$_GET[id]'");
			} else {
				mysql_query("DELETE FROM tb_csr WHERE id_article='$_GET[id]'");
			}
...
elseif ($module=='csr' AND $act=='input'){
	...
	if (!empty($lokasi_file)){
		if ($tipe_file != "image/jpg" AND $tipe_file != "image/jpeg" AND $tipe_file != "image/png"){
			...
			$sSQL = "insert into tb_csr(judul,judul_seo,headline, 
...
// Update artikel
elseif ($module=='csr' AND $act=='update')
{
	$lokasi_file    = $_FILES['fupload']['tmp_name'];
	$tipe_file      = $_FILES['fupload']['type'];
	$nama_file      = $_FILES['fupload']['name'];
	...
	if (empty($lokasi_file)){
		$sSQL = "UPDATE tb_csr SET judul= '$_POST[judul]',
		...
		if ($tipe_file != "image/jpg" AND $tipe_file != "image/jpeg" AND $tipe_file != "image/png"){
			...
			$sSQL = "";
			$sSQL = " select gambar from tb_csr where id_article='$_POST[id]'";
...
elseif ($module=='csr' AND $act=='hapusgambar'){ 
	mysql_query("UPDATE tb_csr set gambar='' WHERE id_article='$_GET[id]'");		  
