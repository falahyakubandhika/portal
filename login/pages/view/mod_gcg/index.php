              ...
              <div class="card card-statistics">
                <div class="card-body">
                  <div class="clearfix">
                      ...
                      <?php
$aksi="pages/view/mod_gcg/aksi_download.php";
if (isset($_GET['act'])){
    switch($_GET['act']){
  // Tampil users_group
    // Tampil User
  ...
  case "tambahdownload":
       $sSQL = "  SELECT (download_id + 1) AS download_id FROM tb_gcg ORDER BY download_id DESC LIMIT 1 ";
    ...
    echo "<h2>New File</h2>
          <form method=POST action='$aksi?module=gcg&act=input' name=formData onSubmit='return CekField();' enctype='multipart/form-data'>
          <table>
      <input name='download_id' type='hidden' value='$download_id'>
          <tr><td>Title</td><td><input type=text name='download_title' size='60'></td></tr>
          <tr><td>Title Id</td><td><input type=text name='download_titleid' size='60'></td></tr>
  ...
  // Form Edit Kategori  
  case "editdownload":
  $sSQL="select * from tb_gcg WHERE download_id='$_GET[download_id]' limit 1";
  $rslt=mysql_query($sSQL) or die ("error query");
  $i=0;
  while ($row=mysql_fetch_assoc($rslt))
  {
    ...
    $download_titleid = $row['download_titleid'];
  ...
  $real_gambar = "../".$download_img;
    ...
    echo "<h2>Edit</h2>
    <form method=POST action='$aksi?module=gcg&act=update' name=formData onSubmit='return CekUpdate();' enctype='multipart/form-data'>
          <table>
      <input name='download_id' type='hidden' value='$download_id'><input name='download_img' type='hidden' value='$download_img'>
          ...
          <tr><td>Title Id</td><td><input type=text name='download_titleid' size='60' value='$download_titleid'></td></tr>
          <tr><td>Seq No</td><td><input type=text name='seq' size='10' value='$seq'></td></tr>
      <tr><td>File PDF</td><td>".$download_img."<a href='$aksi?module=gcg&act=hapusgambar&download_id=$download_id&namafile=$download_img'><img src='images/cross.png' class='tombol'></a></td></tr>
  ...
  echo "<h2>Good Corporate Governance List</h2>
  </div>
          <button  style='margin-bottom:15px' type='button' onclick=\"window.location.href='?module=gcg&act=tambahdownload ';\" class='btn btn-primary btn-fw'> <i class='mdi mdi-file-plus'></i>Add GCG</button>
          ...
          "; 
    $tampil=mysql_query("SELECT * FROM tb_gcg ORDER BY download_id DESC ");
    $no=1;
    ...
    if ($r['download_stsActive'] == '1'){
      ...
      <td>$blokir</td>";   
     echo "
             <td><a href=?module=gcg&act=editdownload&download_id=$r[download_id] title='Edit'><button type='button' class='btn btn-dark btn-fw'>
                          <i class='mdi mdi-cloud-download'></i>Edit</button></a>  
          <a href=$aksi?module=gcg&act=hapus&download_id=$r[download_id] title='Delete'><button type='button' class='btn btn-danger btn-fw'> X Delete</button></a>
           </td>";
