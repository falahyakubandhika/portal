...
if (empty($_SESSION['namauser']) AND empty($_SESSION['passuser'])){
	...
	$act=$_GET['act'];
	// Hapus artikel
	if ($module=='award' AND $act=='hapus'){
		$data=mysql_fetch_array(mysql_query("SELECT gambar,fl_active FROM tb_award WHERE id_article='$_GET[id]'"));
			if ($data['gambar']!=''){
				$gambar = $data['gambar'];
				$remove = "../../../".$gambar;
				...
				mysql_query("DELETE FROM tb_award WHERE id_article='$_GET[id]'");
			} else {
				mysql_query("DELETE FROM tb_award WHERE id_article='$_GET[id]'");
			}
...
elseif ($module=='award' AND $act=='input'){
	...
	if (!empty($lokasi_file)){
		if ($tipe_file != "image/jpg" AND $tipe_file != "image/jpeg" AND $tipe_file != "image/png"){
			echo "<script>window.alert('Upload Gagal, Pastikan File yang di Upload bertipe *.JPG atau PNG');
				window.location=('../../../?module=artikel)
			</script>";
			...
			$sSQL = "insert into tb_award(judul,judul_seo,headline, 
...
// Update artikel
elseif ($module=='award' AND $act=='update')
{
	$lokasi_file    = $_FILES['fupload']['tmp_name'];
	$tipe_file      = $_FILES['fupload']['type'];
	$nama_file      = $_FILES['fupload']['name'];
	...
	if (empty($lokasi_file)){
		...
		if ($tipe_file != "image/jpg" AND $tipe_file != "image/jpeg" AND $tipe_file != "image/png"){
			...
			$sSQL = "";
			$sSQL = " select gambar from tb_award where id_article='$_POST[id]'";
			...
			$sSQL = "UPDATE tb_award SET judul= '$_POST[judul]',
			judul_id= '$_POST[judul_id]',
			judul_seo   = '$judul_seo', 
			headline    = '$_POST[headline]',
			isi_berita  = '$_POST[isi_artikel]',
...
elseif ($module=='award' AND $act=='hapusgambar'){ 
	mysql_query("UPDATE tb_award set gambar='' WHERE id_article='$_GET[id]'");		  
