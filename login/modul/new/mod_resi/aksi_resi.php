...
if ($module=='resi' AND $act=='update'){
   // do nothing here
}
elseif ($module=='resi' AND $act=='updatestatus'){
	$order_no = $_GET['order_no'];
	$no_resi = $_POST['no_resi'];
	//$dt_resi = $_POST['dt_resi'];
	
	$tgl=date('Y-m-d', strtotime($_POST['dt_resi']));
	
	
	$sSQL = " update order_temp set fl_status =4,id_upd='".$_SESSION['namauser']."',dt_upd=now()";
	 $sSQL = $sSQL.", no_resi='".$no_resi."',dt_resi='".$tgl."'";
	...
	/*
	  ...
	  username VARCHAR(47)
	  expedition_id TINYINT
	  no_resi VARCHAR(50)
	  dt_resi  DATETIME 
	*/  
	
	/* Force Insert No Resi */
	$sSQL = "";
	$sSQL = " select o.order_dt, o.username, o.expedition_id,o.id_kota , m.first_name , m.last_name 
	          from order_temp o
	...
	$sSQL = "";
	$sSQL = " insert into tb_resi (order_no,order_dt , username,full_name , expedition_id , no_resi ,dt_resi, id_kota) ";
	$sSQL = $sSQL." values ('".$order_no."','".$order_dt."','".$username."','".$full_name."','".$expedition_id."','";
	$sSQL = $sSQL.$no_resi."','".$tgl."','".$id_kota."')";
	...
	/* Jgn Lupa Email Resi Pengiriman disini */
	$sql2 = mysql_query("select * from company where id='1'");
		 ...
		 $pesan = $pesan."<body><br>";	 
		 $pesan = $pesan."<p><u><strong>".$j2['nama_toko']." RESI PENGIRIMAN </strong></u></p><br>";	 
		...
		$sSQL = " SELECT o.username ,o.order_dt, o.sentaddress,o.zipcode , o.attention_to , o.phone, o.ongkir ,
			...
			$pesan = $pesan.'<table class="shopping-cart-summary" style="color:#000000;font-weight:100">
						 ...
						 $pesan= $pesan."<p> Thank you for shopping with us . We appreciate customer satisfaction </p>";
		...
		/*			
		$to = $email;
		$subject = "Payment Confirmation ".$j2['nama_toko'];
		...
		*/
		...
		/* Email ke member dari salesorder@xxxx.com */				
			$to = $email;
			$subject = "Resi Pengiriman ".$j2['nama_toko'];
			$headers = "From: " . strip_tags($j2['email_pengelola']) . "\r\n";
			$headers .= "Reply-To: ". strip_tags($j2['email_pengelola']) . "\r\n";
	...
	//header('location:../../media.php?module='.$module.'&act=detailorder&id='.$order_id);
	
	echo "<script>
						
						window.location=('../../media.php?module=resi')
	</script>";
